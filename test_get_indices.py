...
"""
        self.df = pandas.read_csv(StringIO(csv_data))
        # df has 17 rows of data (indexed 0-16), with the first row of csv as header.

    def test_empty_filter(self):
        self.assertEqual(get_indices("", self.df), set())

    def test_wildcard_filter(self):
        self.assertEqual(get_indices("*", self.df), set(range(17)))

    def test_single_digit(self):
        self.assertEqual(get_indices("2", self.df), {1})

    def test_multiple_digits(self):
        self.assertEqual(get_indices("1, 5, 10", self.df), {0, 4, 9})

    def test_simple_range(self):
        self.assertEqual(get_indices("5-9", self.df), {4, 5, 6, 7, 8})

    def test_combined_filter(self):
        self.assertEqual(get_indices("1, 5-7, 15", self.df), {0, 4, 5, 6, 14})

    def test_inversion(self):
        """Test inverting a single number after selecting all."""
        self.assertEqual(get_indices("*, !5", self.df), set(range(17)) - {4})

    def test_inverted_range(self):
        """Test inverting a range after selecting all."""
        self.assertEqual(get_indices("*, !5-9", self.df), set(range(17)) - {4, 5, 6, 7, 8})

    def test_inversion_without_base(self):
        """Test that inverting from an empty set results in an empty set."""
        self.assertEqual(get_indices("!5", self.df), set())

    def test_inverted_range_without_base(self):
        """Test that inverting a range from an empty set results in an empty set."""
        self.assertEqual(get_indices("!5-9", self.df), set())

    def test_complex_filter(self):
        self.assertEqual(get_indices("1, 5-9, !7, 15", self.df), {0, 4, 5, 7, 8, 14})

    def test_overlapping_filters(self):
        self.assertEqual(get_indices("1-5, 3-7", self.df), {0, 1, 2, 3, 4, 5, 6})

    def test_empty_parts(self):
        self.assertEqual(get_indices("1,,5", self.df), {0, 4})
